...
/*
 * Fixed **(maybe): when enemy runs into player, player loses a life
 * Fixed: prevents player from jumping fence from wrong direction
 * Added: lives
 * Added: restart option after game ends
 * Added **(incomplete) : possibility to animate up/down enemy movement
 */

// could use 2d arrays
//*Shouldn't be able to jump if behind fence is a rock/tree/other fence/etc.*
//*will be a bug if fence is next to a bridge*
// pause before showing that level is done
// make game unplayable if game over
// reset animation after game loses?
// add up/down enemy animation
...
function loadLevel() {
  ...
  animateEnemy(animateBoxes, 0, "right");/////***********add for up/down movement******
...
function animateEnemy(boxes, index, direction) {
  ...
  if (num < 0) {
    num = 1;
  } else if (num > boxes.length - 1) {
    num = boxes.length - 2;
  } // else if

  nextBox = "" + boxes[num].classList;

  if (nextBox.includes("horse")) {
...
function hitEnemy() {
  ...
  //     loadLevel();
  //   }, 700);
  //   console.log("display lose message");
  // } // if
...
function endGame(state) {
 ...
 clearTimeout(currentAnimation);
  ...
  // end screen for winning
    ...
    document.getElementById("options").innerHTML = "Play Again";
    document.getElementById("end").style.display = "block";
  } // else
} // endGame
